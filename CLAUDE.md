...
- **Templates live in `templatesDir`, not the input tree.** A page's `template` is set either explicitly (`template:` in frontmatter — accepts a string path resolved against `templatesDir`, or a module object via `import`) or by the `default_template` walk: `assembleTree` walks from each module up to the root and uses the first non-undefined `default_template` it finds (starting at the module itself). `index.js` calls `resolveTemplateChain` *after* `assembleTree`, so string values inherited via `default_template` get rewritten to the loaded module too. There's no longer any magic for files literally named `template.md` in the input tree — those are just regular pages.
- **The `child_modules` iterator is non-enumerable**, defined per-instance via `Object.defineProperty`, so spread/`Object.keys` see only the children. Iteration is sorted-by-name.
