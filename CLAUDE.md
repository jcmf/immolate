...
- **`childPages` is rewritten by the tool.** Anything a user file exports under that name is overwritten in tree assembly. Spec-mandated; don't add an "input wins" override.
...
- **The `childPages` iterator is non-enumerable**, defined per-instance via `Object.defineProperty`, so spread/`Object.keys` see only the children. Iteration is sorted-by-name.
