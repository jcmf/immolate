...
- **Builtins ride on `mm`, not on a separate global.** `src/builtins.js` exports helpers — `html(s) → {html: String(s)}` and `makeReadfile({fs, topDir, importerAbsPath, importerDisplay}) → readfile(spec)`. `registry.js` does `mm.html ??= …` and `mm.readfile ??= …` after `Object.assign(mm, compiled)` so the builtins reach user expressions through the same `recma-self` destructure path as ordinary module properties. `??=` means a user export with the same name wins — consistent with the "named exports win" rule for everything on `mm`. `readfile` is sync (JSX expressions render synchronously, so no `fs.promises` here); paths starting with `/` resolve against `topDir`, everything else (including bare names) resolves against the module's own directory — matching POSIX intuition rather than `import`'s npm-flavored "no bare specs" rule.
