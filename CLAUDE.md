...
- **Builtins ride on `mm`, not on a separate global.** `src/builtins.js` exports helpers (currently just `html(s) → {html: String(s)}`); `registry.js` does `mm.html ??= htmlBuiltin` after `Object.assign(mm, compiled)` so the builtin reaches user expressions through the same `recma-self` destructure path as ordinary module properties. `??=` means a user export named `html` wins — consistent with the "named exports win" rule for everything else on `mm`.
